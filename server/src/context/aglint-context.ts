...
export class AglintContext {
    ...
    public static async create(
        ...
        if (!aglint) {
            connection.console.error('[lsp] AGLint module could not be loaded');
            return undefined;
        }

        connection.console.info(`[lsp] AGLint module loaded from ${aglint.modulePath} (version: ${aglint.version})`);
        ...
        const debuggerInstance = new aglint.cli.Debug({
            ...
            printElapsed: false,
            colors: false,
            logger: (message) => connection.console.debug(`[aglint] ${message}`),
        });
