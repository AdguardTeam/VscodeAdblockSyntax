...
module.exports = {
    ...
    '**/*.ts': [
        ...
        (files) => {
            const filesToCheck = files.filter(file => !file.includes('rspack.config.ts') && !file.includes('vitest.config.ts'));
        ...
        (files) => {
            const testableFiles = files.filter(file => !file.includes('rspack.config.ts') && !file.includes('vitest.config.ts'));
